...
let resetCordovaFolder = function (callback) {
  alert('Phonegap directory reset ongoing - please wait ...')
  fs.readdir(binDir, function (err, files) {
    if (err) {
      alert('Phonegap directory reset failed.', 'issue')
    } else {
      let filesToDelete = []
      for (let f = 0; f < files.length; f++) {
        if (files[f] !== 'phonegap' && files[f] !== 'phonegap.js') {
          filesToDelete.push(files[f])
        }
      }
      deleteFiles(filesToDelete, function () {
        alert('Phonegap directory reset done.')
        callback()
      })
    }
  })
...
let createCordovaProject = function (callback) {
  ...
  cmd(binDir, 'phonegap create .temp', function () {
    fs.copy(abs(binDir, '.temp'), binDir, function (err) {
      if (!err) {
        fs.remove(abs(binDir, '.temp'), function (err) {
          if (!err) {
            alert('Phonegap project folder creation done.')
            callback()
          } else {
            alert('Failed to remove Phonegap temp folder.', 'issue')
          }
        })
      } else {
        alert('Failed to copy Phonegap project folder.', 'issue')
      }
    })
  }, function () {
    alert('Failed to create Phonegap project folder.', 'issue')
  })
}
let updateCordovaConfig = function (callback) {
  alert('Phonegap configuration update ongoing - please wait ...')
  ...
  fs.writeFile(abs(binDir, 'config.xml'), xmlContent, function (err) {
    if (!err) {
      ...
      alert('Phonegap configuration update failed.', 'issue')
    }
  })
}
let updateWwwFolder = function (callback) {
  alert('Phonegap build folder update ongoing - please wait ...')
  fs.remove(wwwFolder, function (err) {
    if (!err) {
      ...
      alert('Failed to reset Phonegap build folder.', 'issue')
    }
  })
}
let installCordovaPlugins = function (callback, pluginList) {
  alert('Phonegap plugin installation ongoing - please wait ...')
  ...
  if (Array.isArray(pluginList) && pluginList.length > 0) {
    ...
    alert('Phonegap plugin installation done.')
    callback()
  }
}
let addCordovaPlatforms = function (callback) {
  ...
  if (env.arg.ios === true || env.arg.xcode === true) {
    cmd(binDir, 'phonegap platform add ios', function () {
      ...
      // Alert
      alert('Phonegap platform installation done for Android.')
      // Callback
      callback()
    }, function () {
      alert('Phonegap platform installation failed for iOS.', 'issue')
    })
  } else {
    cmd(binDir, 'phonegap platform add android', function () {
      fs.mkdir(abs(binDir, 'platforms/android/.idea'), function (err) {
        if (!err) {
          alert('Phonegap platform installation done for Android.')
          callback()
        } else {
          alert('Failed to create .idea folder for Phonegap Android platform.', 'issue')
        }
      })
      alert('Phonegap platform installation done for Android.')
      callback()
    }, function () {
      alert('Phonegap platform installation failed for Android.', 'issue')
    })
  }
}
